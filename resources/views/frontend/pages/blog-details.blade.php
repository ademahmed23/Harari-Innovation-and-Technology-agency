        ...
        <div class="fp__breadcrumb_overlay">
            <div class="container">
                <div class="fp__breadcrumb_text">
                    <h1>{{__('frontend.blog details')}}</h1>
                    <ul>
                        <li><a href="index.html">{{__('frontend.home')}}</a></li>
                        <li><a href="#">{{__('frontend.blog details')}}</a></li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-xl-8 col-lg-8">
                    <div class="fp__blog_det_area">
                        ...
                        <div class="fp__blog_details_text wow fadeInUp" data-wow-duration="1s">
                            <ul class="details_bloger d-flex flex-wrap">
                                <li><i class="far fa-user"></i> {{__('frontend.By')}} {{ $blog->user->name }}</li>
                                <li><i class="fas fa-comments"></i> {{ $blog->comments_count }} {{__('frontend.comment')}}</li>
                        ...
                        <li>
                            <a href="{{ route('blogs.details', $nextBlog->slug) }}">
                                ...
                                <p>{{ truncate($nextBlog->title) }}
                                    <span> <i class="far fa-long-arrow-left"></i> {{__('frontend.Previous')}}</span>
                                </p>
                            </a>
                        </li>
                        @endif
                    ...
                    <div class="fp__comment mt_100 xs_mt_70 wow fadeInUp" data-wow-duration="1s">
                        <h4>{{ count($comments) }} {{__('frontend.Comments')}}</h4>
                    ...
                    <div class="comment_input mt_100 xs_mt_70 wow fadeInUp" data-wow-duration="1s">
                        <h4>{{__('frontend.Leave A Comment')}}</h4>
                        <p>{{__('frontend.Required fields are marked *')}}</p>
                        <form action="{{ route('blogs.comment.store', $blog->id) }}" method="POST">
                            @csrf
                            <div class="row">

                                <div class="col-xl-12">
                                    <label>{{__('frontend.comment *')}}</label>
                                    ...
                                    <button type="submit" class="common_btn mt_20">{{__('frontend.Submit comment')}}</button>
                                </div>
                            </div>
                        </form>
                    </div>
